...
class MapCreator {
	...
	private BufferedImage koopafly, koopaflyRight;
	private BufferedImage chomper;
	...
	Map createMap(String mapPath, double timeLimit) {
		...
		int bowser = new Color(255, 9, 255).getRGB();
		int koopafly = new Color(100,100,100).getRGB();
		int chomper = new Color(9, 255, 0).getRGB();
		...
		for (int x = 0; x < mapImage.getWidth(); x++) {
			for (int y = 0; y < mapImage.getHeight(); y++) {
				...
				if (currentPixel == ordinaryBrick) {
					Brick brick = new OrdinaryBrick(xLocation, yLocation,
							this.ordinaryBrick);
					createdMap.addBrick(brick);
				} else if (currentPixel == surpriseBrick) {
					Prize prize = generateRandomPrize(xLocation, yLocation);
					Brick brick = new SurpriseBrick(xLocation, yLocation,
							this.surpriseBrick, prize);
					createdMap.addBrick(brick);
					...
					Brick brick = new GroundBrick(xLocation, yLocation,
							this.groundBrick);
					createdMap.addGroundBrick(brick);
				} else if (currentPixel == goomba) {
					Enemy enemy = new Goomba(xLocation, yLocation,
							this.goombaLeft);
					((Goomba) enemy).setRightImage(goombaRight);
					createdMap.addEnemy(enemy);
				}
				else if (currentPixel == koopafly) {
					Enemy enemy = new KoopaFly(xLocation, yLocation, this.koopafly);
					((KoopaFly)enemy).setRightImage(koopaflyRight);
					createdMap.addEnemy(enemy);
				}
				else if (currentPixel == chomper) {
					spike spike = new spike(xLocation, yLocation, this.chomper);
					createdMap.addEnemy(spike);
				}
				// bowser
					...
					EndFlag endPoint = new EndFlag(xLocation + 24, yLocation,
							endFlag);

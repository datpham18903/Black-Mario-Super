...
public class Mario extends GameObject {
	...
	public Mario(double x, double y) {
		...
		this.setStyle(
				this.marioForm.getCurrentStyle(this.toRight, false, false));
	}
	@Override
	public void draw(Graphics g) {
		boolean movingInX = this.getVelX() != 0.0;
		boolean movingInY = this.getVelY() != 0.0;
		this.setStyle(this.marioForm.getCurrentStyle(this.toRight, movingInX,
				movingInY));
		super.draw(g);
	}
	...
	public boolean onTouchEnemy(GameEngine engine, Enemy enemy) {
		if (enemy instanceof Bowser) {
			--this.remainingLives;
			this.marioForm = this.marioForm
					.onTouchEnemy(engine.getImageLoader());
			this.setDimension(48, 48);
			return true;
		} else {
	...
	public boolean isFire() {return this.marioForm.isFire();
	}
